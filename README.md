...
+---------------------------------------------------------------------------------+
                      ...
                      | (codex, acp profile, pi, claude, devin) |
                      +---------------------------------------+
```
...
| **Default Agent** | — | `TODEX_AGENTD_DEFAULT_AGENT` | `codex` | Default provider (`codex`, `acp`, `pi`, `claude-code`, `grok-build`, `devin`). |
...
- `GET /v2/providers/versions`: Inspect the configured backend host's Codex, Pi, Claude Code, Grok Build, Devin, and external ACP CLI versions.
